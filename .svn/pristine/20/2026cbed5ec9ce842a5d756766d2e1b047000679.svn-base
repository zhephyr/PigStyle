...
Public Class ModifyServerList_Xen
        ...
        If Page.IsValid Then
            ...
            If Not match Then
                Using connection As New SqlConnection("server=VG-UNITEDDB;database=freshdb;Uid=sa;Pwd=2011Sql;")
                    Dim addCmd As New SqlCommand("INSERT INTO servername_xen VALUES (@AddServer, @AddIP);", connection)
    ...
    Public Sub DelClick(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs)
        ...
        If Page.IsValid Then
            ...
            If match Then
                Using connection As New SqlConnection("server=VG-UNITEDDB;database=freshdb;Uid=sa;Pwd=2011Sql;")
                    Dim DeleteCmd As New System.Data.SqlClient.SqlCommand("DELETE FROM servername_xen WHERE (server=@DeleteServer);", connection)
    ...
    Private Function AddLookup() As Boolean
        Using connection As New SqlConnection("server=VG-UNITEDDB;database=freshdb;Uid=sa;Pwd=2011Sql;")
            Dim command As New SqlCommand("SELECT COUNT(*) FROM servername_xen WHERE (server=@SelectServer AND ip=@SelectIP);", connection)
    ...
    Private Function DelLookup() As Boolean
        Using connection As New SqlConnection("server=VG-UNITEDDB;database=freshdb;Uid=sa;Pwd=2011Sql;")
            Dim command As New SqlCommand("SELECT COUNT(*) FROM servername_xen WHERE (server=@SelectServer);", connection)
    ...
    Private Sub LoadGrid()
        Using conn As New System.Data.SqlClient.SqlConnection("server=VG-UNITEDDB;database=freshdb;Uid=sa;Pwd=2011Sql;")
            conn.Open()
            Dim cmd As New SqlCommand("SELECT * FROM servername_xen ORDER BY server;", conn)
            Try
